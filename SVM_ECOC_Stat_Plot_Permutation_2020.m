...
%% Parameters
% load('F:\exp1_lxs\Decoding\TD\Alpha_P1_Analysis.mat');

% Subject List 
subList = [1:6 8 9 11:14 16:36];
% subList =[2 4 5 7:10 12 14:17 21:23 25 28:30 32:37];
...
for sub = 1:Nsub 
    ...
    readThis =strcat(fileLocation,fName,num2str(isub),'.mat');
    load(readThis)
...
boundedline(tm,subAverage,seAverage,'cmap',cl(246,:),'alpha','transparency',0.35)
set(gca,'YLim',[0.1 0.17]);
set(gca,'XLim',[-200 2200]);
line([tm(1),tm(length(tm))],[chancelvl,chancelvl],'Color','black','LineStyle','--'); %chance line
...
if ClusterMassAnalysis == 1
    ...
    load('ValidDecoding_ERPbased_Target_T_mass.mat')
    ...
    ax = gca;
    ax.YLim = [0.1, 0.17];
    ax.YTick = [0.1,0.13,0.15,0.17];
%     ax.XTick = [1 11 21 31 41 51 61 70];
%     ax.XTickLabel = {'-200','0','200','400','600','800','1000','1200'};
